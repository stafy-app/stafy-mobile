...
| `app/(auth)/register.tsx` | Live | 4-step onboarding wizard (welcome → role → identity → account) instead of a single form; local state accumulates across steps, no network call until the last one. Picking `manager` on the role step redirects straight to `/manager-mobile-blocked`. Identity step enforces min 2 / max 30 on both names (mirrors backend `UserRegisterIn`); "Nume" → `last_name`, "Prenume" → `first_name` (state vars `lastName`/`firstName` named for the backend field). Last step runs Firebase (`createUserWithEmailAndPassword`) then `POST /api/v1/auth/register` with the ID token; a backend failure or `auth/email-already-in-use` raises `OrphanRegistrationError` → popup routing to `/login` (no `Alert.alert`, no inline retry). See `docs/modules/auth.md` Flow 1 |
...
- **Login/register are Firebase-first**: `login`/`register` in `UserContext.tsx` call the Firebase SDK directly (`signInWithEmailAndPassword` / `createUserWithEmailAndPassword`), then send the resulting ID token to the backend. `/api/v1/auth/register`'s JSON body is `{first_name, last_name, role}` only — email/password never reach the backend. `register()` / `completeRegistration()` take `{firstName, lastName, ...}` (backend-field names), not the Romanian `{name, surname}` — the register wizard maps "Prenume" → `firstName`, "Nume" → `lastName`, same as `stafy-web-app`. See `stafy-mobile/docs/modules/auth.md`.
...
❌ **Registration is two sequential calls** (`createUserWithEmailAndPassword` then `POST /api/v1/auth/register`). If the backend call fails after the Firebase account was created, the Firebase user is deliberately left in place (no rollback) — see `docs/modules/auth.md` "Special Aspects → Orphan registration". `register()` raises `OrphanRegistrationError` both here and on `auth/email-already-in-use` (retry-after-orphan); `register.tsx` shows a popup routing to `/login`, from where `login()` re-raises it on the 404 and `login.tsx` routes to `app/(auth)/complete-registration.tsx`. Cold-start hydration does not route there directly — a relaunch while orphaned still needs one login attempt first.
